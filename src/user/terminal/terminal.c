...
#define TERM_TRAIN_CALIB_DATA_OFF    TERM_TRAIN_STATE_ERR_OFF + 10
    ...
    FOREVER {
        ...
        switch(data->command) {
            ...
            case TERMINAL_DISPLAY_TRAIN_CALIBRATION:
                //TODO
                handle_display_average_velocity_information(data->num1, (avg_velocity_t*)data->average_velocity_info);
                break;
            default:
                ASSERT(0);
                break;
...
void handle_display_average_velocity_information(int16_t train, avg_velocity_t* average_velocity_info) {
    term_save_cursor();
    term_move_cursor(TERM_TRAIN_STATE_START_COL + TERM_TRAIN_CALIB_DATA_OFF, TERM_TRAIN_STATE_START_ROW);

    printf(COM2, "Calibration Information Train: %d\r\n", train);
    term_move_to_column(TERM_TRAIN_STATE_START_COL + TERM_TRAIN_CALIB_DATA_OFF);

    int i, j;
    for(i = 0; i < 80; ++i) {
        char first_sensor_letter = 'A' + (i / 8);
        int first_sensor_num = i % 8;

        for(j = 0; j < MAX_AV_SENSORS_FROM; ++j) {
            avg_velocity_t* avg_velocity = &average_velocity_info[i][j];

            if(avg_velocity->average_velocity_count == 0) {
                printf(COM2, "%c%d to %s: %d Iterations: %d\r\n", first_sensor_letter, first_sensor_num, 
                    avg_velocity->average_velocity, avg_velocity->average_velocity_count);
                term_move_to_column(TERM_TRAIN_STATE_START_COL + TERM_TRAIN_CALIB_DATA_OFF);
            }
        }
    }
